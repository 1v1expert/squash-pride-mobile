...
const Profile = () => {
    const {logout, updateUser, user, sendFeedback} = useUser();
    ...
    const feedbackMethods = useForm<FeedbackForm>();
    ...
    const pressSendFeedback = (values: FeedbackForm) => {
        sendFeedback({
            title: values.title,
            description: values.description,
        });
        changeMode();
        feedbackMethods.reset({
            title: '',
            description: ''
        });
    }
                        ...
                        <VStack flex={1} width="$full" justifyContent="space-between">
                            <VStack pt={20} paddingHorizontal={30}>
                                <CustomInput
                                    name="title"
                                    placeholder={t(
                                        'public.registrationScreen.feedbackTitle',
                                    )}
                                    variant="secondary"
                                />
                                <CustomInput
                                    name="description"
                                    placeholder={t(
                                        'public.registrationScreen.feedbackDescription',
                                    )}
                                    variant="secondary"
                                />
                                <HStack height={perfectSize(50)} justifyContent="center" space="xl">
                                    ...
                                    />
                                    <CustomButton
                                        title={t('private.profileScreen.cancel')}
